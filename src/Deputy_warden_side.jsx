...
function Deputy_warden_side({ onLogout }) {
    ...
    return (
        <div className="h-screen w-full flex flex-col font-sans bg-[#0a1628] text-white overflow-hidden">
            ...
            {/* ── Header ── */}
            <header className="w-full flex items-center justify-between px-4 sm:px-8 py-3 sm:py-5 border-b border-white/5 bg-[#0a1628]/80 backdrop-blur-xl sticky top-0 z-50 gap-3 flex-wrap">
                <div className="flex items-center gap-3 sm:gap-4">
                    <img src={logo} alt="GCES Logo" className="w-9 h-9 sm:w-11 sm:h-11 object-contain" />
                    <div className="flex flex-col leading-tight">
                        <span className="text-[10px] sm:text-xs font-semibold tracking-[0.25em] text-teal-400/80 uppercase">Deputy Warden Panel</span>
                        <span className="text-lg sm:text-xl font-black text-white tracking-widest uppercase">MessReduction</span>
                    </div>
                </div>
                ...
                {/* View Toggle */}
                <div className="flex bg-[#0f1f38]/60 p-1 sm:p-1.5 rounded-2xl border border-white/5 backdrop-blur-sm">
                    <button onClick={() => setView("dashboard")} className={`flex items-center gap-1.5 sm:gap-2 px-4 sm:px-6 py-2 sm:py-2.5 rounded-xl text-xs font-black tracking-widest uppercase transition-all duration-300 ${view === "dashboard" ? "bg-teal-500 text-slate-900 shadow-[0_0_20px_rgba(45,212,191,0.3)]" : "text-white/40 hover:text-white"}`}>
                        <FiBarChart2 size={14} /> <span className="hidden sm:inline">Dashboard</span><span className="sm:hidden">Dash</span>
                    </button>
                    <button onClick={() => setView("requests")} className={`flex items-center gap-1.5 sm:gap-2 px-4 sm:px-6 py-2 sm:py-2.5 rounded-xl text-xs font-black tracking-widest uppercase transition-all duration-300 ${view === "requests" ? "bg-teal-500 text-slate-900 shadow-[0_0_20px_rgba(45,212,191,0.3)]" : "text-white/40 hover:text-white"}`}>
                ...
                {onLogout && (
                    <button onClick={onLogout} className="flex items-center gap-2 px-3 sm:px-4 py-2 border border-rose-500/20 rounded-xl text-xs font-black text-rose-400 uppercase tracking-widest hover:bg-rose-500 hover:text-white hover:border-rose-500 transition-all">
                        <FiUsers size={13} /> <span className="hidden sm:inline">Logout</span>
                    </button>
                )}
            </header>

            {/* ── Main Content ── */}
            <main className="flex-1 overflow-y-auto p-4 sm:p-8 lg:p-12 [&::-webkit-scrollbar]:hidden">
